@echo off
cd /d "%~dp0"
call "%~dp0_archive_python_env.bat"
echo ========================================
echo  Monthly channel archive
echo ========================================
echo.
echo Primary driver for routine / whole-channel backup from channels_input.txt.
echo Same stack as monthly_watch_later_archive.bat: yt-dlp.conf, cookies, EJS,
echo per-run logs under logs\archive_run_^<UTC^>\, verification before archiving IDs.
echo.
echo Bare channel URLs (no /videos) expand to /videos + /shorts by default
echo ^(set ARCHIVE_CHANNEL_EXPAND_TABS=0 to disable^).
echo.
echo This script includes:
echo - File size verification after download ^(same as playlist driver^)
echo - Deferred channels_downloaded.txt ^(only verified completes^)
echo - Per-run manifest, issues, summary.txt, report.html
echo - Pointer: logs\latest_run_channel.txt ^(playlist runs use logs\latest_run.txt^)
echo.
if /i not "%ARCHIVE_CONSOLE_UNATTENDED%"=="1" (
    echo Press any key to start...
    pause >nul
)

:download_start
echo.
echo Starting channel archive run...
echo.
echo Environment toggles ^(optional^):
echo   SKIP_PIP_UPDATE=1           skip pip self-upgrade ^(default when unset^)
echo   SKIP_YTDLP_UPDATE=1          skip yt-dlp pip install before run
echo   ARCHIVE_DRY_RUN=1            pass --simulate to yt-dlp
echo   ARCHIVE_CHANNEL_EXPAND_TABS=0   do not split bare channel URLs into /videos + /shorts
echo   ARCHIVE_PAUSE_ON_COOKIE_ERROR=1   pause on likely cookie/auth yt-dlp lines ^(see logs^)
echo   ARCHIVE_COOKIE_AUTH_POLL_SEC=N    optional mtime poll on cookies.txt every N seconds
echo   Cookie source: edit yt-dlp.conf ^(--cookies file vs --cookies-from-browser^)
echo   ARCHIVE_OUT_CHANNEL=    optional: output root for channels ^(set by Archive Console^)
echo   ARCHIVE_PIP_VERBOSE=1        full pip output ^(default: pip -q for less spam^)
echo.

if /i "%ARCHIVE_PIP_VERBOSE%"=="1" (
    set "ARCHIVE_PIP_QUIET="
) else (
    set "ARCHIVE_PIP_QUIET=-q"
)

if defined ARCHIVE_OUT_CHANNEL (
    echo [archive] ARCHIVE_OUT_CHANNEL set — channel downloads use that folder tree.
)

if not defined SKIP_PIP_UPDATE set "SKIP_PIP_UPDATE=1"
if /i "%SKIP_PIP_UPDATE%"=="1" (
    "%ARCHIVE_PY%" "%~dp0archive_print_role.py" skip "[archive] Skipping pip self-upgrade (SKIP_PIP_UPDATE=1)"
) else (
    echo [archive] Upgrading pip ^(python -m pip install --upgrade pip^)...
    "%ARCHIVE_PY%" -m pip install %ARCHIVE_PIP_QUIET% --upgrade pip --disable-pip-version-check
    if errorlevel 1 (
        "%ARCHIVE_PY%" "%~dp0archive_print_role.py" warn "WARNING: [archive] pip self-upgrade failed; continuing with existing pip."
    ) else (
        "%ARCHIVE_PY%" "%~dp0archive_print_role.py" ok "[archive] pip self-upgrade finished OK."
    )
    echo.
)

if /i "%SKIP_YTDLP_UPDATE%"=="1" (
    "%ARCHIVE_PY%" "%~dp0archive_print_role.py" skip "Skipping yt-dlp update (SKIP_YTDLP_UPDATE=1)"
) else (
    echo Updating yt-dlp via pip ^(includes yt-dlp-ejs for YouTube challenges^)...
    "%ARCHIVE_PY%" -m pip install %ARCHIVE_PIP_QUIET% -U --disable-pip-version-check "yt-dlp[default]"
    if errorlevel 1 (
        "%ARCHIVE_PY%" "%~dp0archive_print_role.py" warn "WARNING: pip could not update yt-dlp; continuing with the already-installed version."
    ) else (
        "%ARCHIVE_PY%" "%~dp0archive_print_role.py" ok "[archive] yt-dlp pip update finished OK."
    )
    echo.
)

if exist "channels_downloaded.txt" (
    copy "channels_downloaded.txt" "channels_downloaded_backup.txt" >nul
    echo Created backup of channels_downloaded.txt
)

echo Per-run logs: logs\archive_run_^<UTC_time^>\
echo Channel pointer: logs\latest_run_channel.txt
echo Playlist pointer ^(Watch Later^): logs\latest_run.txt
echo Checklist / failure families: ARCHIVE_PLAYLIST_RUN_LOGS.txt
echo.

"%ARCHIVE_PY%" "%~dp0archive_channel_run.py"
set "YTDLP_RC=%ERRORLEVEL%"

set "LOGDIR="
if exist "logs\latest_run_channel.txt" (
    set /p LOGDIR=<logs\latest_run_channel.txt
)
if defined LOGDIR (
    echo.
    echo Latest channel run log directory:
    echo   %LOGDIR%
) else (
    echo.
    echo Latest channel run log directory: see Python output above
)

if not "%YTDLP_RC%"=="0" (
    echo.
    echo ========================================
    echo  Channel archive completed with errors
    echo ========================================
    echo.
    call :print_log_paths
    echo See ARCHIVE_PLAYLIST_RUN_LOGS.txt ^(channel section^).
    echo.
    echo Press any key to exit...
    if /i not "%ARCHIVE_CONSOLE_UNATTENDED%"=="1" pause >nul
) else (
    echo.
    echo ========================================
    echo  Channel archive finished ^(exit 0^)
    echo ========================================
    echo.
    echo See manifest for verified downloads; archive file updated only for verified files.
    echo.
    call :print_log_paths
    echo See ARCHIVE_PLAYLIST_RUN_LOGS.txt ^(channel section^).
    echo.
    echo Press any key to exit...
    if /i not "%ARCHIVE_CONSOLE_UNATTENDED%"=="1" pause >nul
)

exit /b %YTDLP_RC%

:print_log_paths
if defined LOGDIR (
    echo See: "%LOGDIR%\manifest.csv"
    echo See: "%LOGDIR%\issues.csv"
    echo See: "%LOGDIR%\summary.txt"
    echo See: "%LOGDIR%\report.html"
    echo See: "%LOGDIR%\rerun_urls.txt"
    echo See: "%LOGDIR%\run.log"
) else (
    echo See logs\latest_run_channel.txt then manifest.csv, issues.csv, etc.
)
exit /b 0
